...
class AC(nn.Module):
    ...
    def act_max(self, img, stat):
        ...
        p = self.actor(hid)
        f = self.f(hid)
        if p.max()-(1-p.max()-p.min()) >= temp or self.last == -20:
            action = p.argmax()
        else:
            action = self.last
        self.last = action
...
def run():
    ...
    closes = []
    while not done:
        action, f = ac.act_max(img, stat)
        actions.append(1-action)
        fs.append(f)
        (img, stat), done = env.step(action, f)
        f_thres.append(env.f)
        moneys.append(env.money/init_money-1)
        closes.append(env.stats.iloc[env.cur_pos-1]['close'])
    rewards = env.reward_list

    return closes, moneys, actions
        
if __name__ == '__main__':
    x = []
    clo, profit, acts = run()
    x = [clo, profit, acts]
    df = pd.DataFrame(np.array(x).transpose(), columns=['prices', 'profit', 'signal'])
    df.to_csv(f"price-profit＿max.csv")